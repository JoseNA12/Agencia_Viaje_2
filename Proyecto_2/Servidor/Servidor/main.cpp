...
void TareaProgramada2::leerArchivoViaje() {
	...
	while (!archivoViaje.eof()) {
		...
		if (ListaIDViajes.buscarElemento(pIdViaje) != true) {
			...
			arbolBinario.InsertaNodoBinarioBusqueda(num, pIdViaje, pOrigen, pDestino, pFechaSalida, pFechaLlegada, pPrecio, pNumPlazas, cantPaquetes);
		}
...
void TareaProgramada2::leerArchivoHotel() {
	...
	while (!archivoHotel.eof()) {
		...
		if (ListaIDHoteles.buscarElemento(pIdHotel) != true) {
			...
			arbolAVL.InsertaNodoAVL(num, pIdHotel, pNombre, pCategoria, pCiudad, pPrecioHabIndiv, pPrecioHabDoble,
...
void TareaProgramada2::leerArchivoTransporte() {
	...
	while (!archivoTransporte.eof()) {
		...
		if (ListaIDTransportes.buscarElemento(pIdTransporte) != true) {
			...
			arbolRN.insertarValorNodoRN(num, pIdTransporte, pTipoTransporte, pOrigen, pDestino, pFechaSalida, pHoraSalida,
...
void TareaProgramada2::leerArchivoOfertaHotel() {
	...
	while (!archivoOfertaHoteles.eof()) {
		...
		if (ListaIDOfertaHoteles.buscarElemento(llaveID) != true) {
			...
			arbolAA.IniciarInsercionAA(num, pIdViaje, pIdHotel, pIDoferta, pPrecioHabIndiv, pPrecioHabDoble, pCantOfertasIndivi, pCantOfertasDobles);
	...
	if (opcionConsulta == "0") {
		...
		if (auxCliente != NULL) {
			...
			if (auxBinario != NULL) {
				...
				return "false,n*** Error inesperado!. El cliente ya reservo pero no tiene viaje asociado ***.";
			}
		}
		else {
...
string TareaProgramada2::RealizarReservacionViaje1(string opcionViaje) {
	...
	cout << "Estoy en reserva viajes 8" << endl;
	cout << "Opcion viaje" << opcionViaje << endl;
	NodoBinarioBusqueda *auxViaje;
	auxViaje = buscarViaje(opcionViaje);
	string s;
...
string TareaProgramada2::RealizarReservacionHotel3(string info)
{
	...
	return "true,n*** Se ha realizado la reservacion!. ***";
...
int main() {
	...
	if (WSAStartup(DllVersion, &wsaData) != 0)						//Pregunto si hay error al iniciar el servidor
	{
		...
		cout << "Servidor conectado exitosamente" << endl;
	}
